...
export class GuitarTuning {
   ...
   */
  private notesSharp = ['E', 'F', 'F♯', 'G', 'G♯', 'A', 'A♯', 'B', 'C', 'C♯', 'D', 'D♯']
  /**
   ...
   */
  constructor(tuning: [number, number, number, number, number, number], transpose: number = 0) {
    this.tuning = tuning
   ...
   */
  getTuningNotes(options: TuningNotesMethodOptions = { asArray: false, transposed: false, forceSharp: false }) {
    ...
    if (options.asArray) {
      return [this.notesSharp[EString], this.notesSharp[AString], this.notesSharp[DString], this.notesSharp[GString], this.notesSharp[BString], this.notesSharp[eString]]
    }
  ...
  getTuningName(options: TuningNameMethodOptions = { transposed: false, forceSharp: false }) {
    ...
    if (this.tuning[0] === this.tuning[1] && this.tuning[1] === this.tuning[2] && this.tuning[2] === this.tuning[3] && this.tuning[3] === this.tuning[4] && this.tuning[4] === this.tuning[5]) {
      ...
      if (this.transpose >= 0 || options.forceSharp) {
        return options.transposed ? `${this.notesSharp[this.ValidateTuning(this.tuning[0] + this.transpose)]} Standard` : `${this.notesSharp[this.ValidateTuning(this.tuning[0])]} Standard`
      }

      return options.transposed ? `${this.notesFlat[this.ValidateTuning(this.tuning[0] + this.transpose)]} Standard` : `${this.notesFlat[this.ValidateTuning(this.tuning[0])]} Standard`
    }
    ...
    if (this.tuning[0] === -2 && this.tuning[1] === this.tuning[2] && this.tuning[2] === this.tuning[3] && this.tuning[3] === this.tuning[4] && this.tuning[4] === this.tuning[5]) {
      if (this.transpose >= 0 || options.forceSharp) {
        return options.transposed ? `Drop ${this.notesSharp[this.ValidateTuning(this.tuning[0] + this.transpose)]}` : `Drop ${this.notesSharp[this.ValidateTuning(this.tuning[0])]}`
      }

      return options.transposed ? `Drop ${this.notesFlat[this.ValidateTuning(this.tuning[0] + this.transpose)]}` : `Drop ${this.notesFlat[this.ValidateTuning(this.tuning[0])]}`
    }
