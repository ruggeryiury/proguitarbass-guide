...
const HeaderMenu = ({ activated, setHeaderMenu }: HeaderMenuProps) => {
  ...
  return (
    <>
      <div
        id="HeaderMenuBG"
        className="fixed bg-main-black/80 z-20 w-full h-screen duration-300  backdrop-blur-sm"
        style={{ left: '-100%' }}
      >
        <div className="w-[10%] ml-auto items-center py-2">
          <button
            ...
            className="w-fit"
            onClick={(ev) => setHeaderMenu(false)}
          >
      ...
      <div
        ...
        style={{ left: '-100%' }}
        className="fixed items-start bg-gradient-to-br from-[#111]/90 to-[#121217]/90 z-30 w-[90%] h-screen duration-300 px-4 py-4 overflow-y-auto"
      >
        ...
        <Topic title={HeaderConfig.basics.title}>
          {PageNames.map((value, index) => {
            ...
            return content.type === 'basics' ? (
              <TopicLink
                key={`basicsButtonMobile${index}`}
                index={content.index ? true : false}
                to={content.url}
                action={setHeaderMenu}
              >
                {content.title}
              </TopicLink>
            ) : null
        ...
        <Topic title={HeaderConfig['notes-and-markers'].title}>
          {PageNames.map((value, index) => {
            ...
            return content.type === 'notes-and-markers' ? (
              <TopicLink
                key={`notesAndMarkersButtonMobile${index}`}
                index={content.index ? true : false}
                to={content.url}
                action={setHeaderMenu}
              >
        ...
        <Topic title={HeaderConfig.mastering.title}>
          {PageNames.map((value, index) => {
            ...
            return content.type === 'mastering' ? (
              <TopicLink
                key={`masteringButtonMobile${index}`}
                index={content.index ? true : false}
                to={content.url}
                action={setHeaderMenu}
              >
