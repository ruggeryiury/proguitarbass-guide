...
export const HeaderMenuTopicLink = ({ children, action, to, index }: HeaderMenuTopicLinkProps) => {
  ...
  return (
    <Link
      to={to}
      onClick={(ev) => action(false)}
      className={`w-fit pl-4 my-1 text-left leading-none mb-2 last:mb-4 ${Context.currentPage === id ? 'text-cyan-500 drop-shadow-[0_2px_2px_#007fff2f]' : 'text-neutral-300'}`}
    >
