        ...
        <>
            ...
            <main className="flex-col laptop:flex-row w-full mobile:landscape:h-[88%] mobile:portrait:h-[92%] laptop:landscape:h-[94%] tablet-md:portrait:h-[94%] justify-center overflow-y-auto laptop:overflow-y-hidden animate__animated animate__fadeIn animate__faster">
                <div className="hidden laptop:flex h-full w-full laptop:w-[22%] py-6 px-6 tablet-md:px-12 laptop:px-8 border-r-0 laptop:border-r-2 border-main-white/10">
                    ...
                    {
                        config.anchors ? config.anchors.map((value, index) => {
                            return <HashLink className='pl-2 laptop:text-sm laptop-lg:text-base desktop:text-lg !leading-[1.10] text-left mb-2 w-fit hover:text-cyan-500' to='' key={`AnchorsLink${index}`} elementId={`#Anchor_${value.id}`} smooth>{value.title.toUpperCase()}</HashLink>
                        }) : null
                    }
                </div>
                                ...
                                <div className='mb-6 bg-main-white/10 py-6 px-2 rounded-md'>
                                    {
                                        config.anchors.map((value, index) => {
                                            return <HashLink className='flex-row pl-2 laptop:text-sm laptop-lg:text-base desktop:text-lg !leading-[1.10] text-left mb-2 w-fit hover:text-cyan-500 last:mb-0' to='' key={`AnchorsLink${index}`} elementId={`#Anchor_${value.id}`} smooth>
